<div align="center">
  ...
  <p>
    ...
    <a href="#vitrine-de-código">Vitrine de código</a> •
...
> [!NOTE]
> **Este é o repositório de vitrine.** O Zelo roda em produção num escritório de
> contabilidade, com dados fiscais de clientes reais, e por isso o repositório de
> desenvolvimento é privado. Aqui ficam a documentação, as telas e os módulos que
> dá para publicar inteiros.
...
| **CI com paridade de banco** | Lint + suíte em SQLite em todo PR; a suíte inteira contra **MySQL 8.0** roda quando há alteração de backend, banco ou testes, além de uma execução noturna e manual, pegando divergência de enum, colação e tipo antes da produção e validando a migração idempotente (`upgrade → downgrade → upgrade`). |
...
Módulos publicados inteiros, com o raciocínio junto. Cada um roda sozinho.

| Módulo | O que demonstra |
| --- | --- |
| [**Barreira de dados sensíveis**](vitrine/dados-sensiveis/) | Um hook de pre-commit que recusa CNPJ, CPF e chave de NF-e reais antes do commit — validando dígito verificador, lendo o índice do git em vez da árvore de trabalho, e falhando fechado. Escrito depois de um vazamento de verdade. |
